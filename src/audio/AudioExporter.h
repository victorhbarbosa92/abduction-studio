...
extern float track_offsets[8];
extern StemSeparationEngine* g_ai_engine;
...
public:
    static void ExportMixdown(const std::string& filepath) {
        ...
        if (g_ai_engine == nullptr || !g_ai_engine->hasFinished()) {
        ...
        bool any_solo = false;
        for(int i=0; i<8; i++) if (track_solos[i]) any_solo = true;
