...
print("[TEST] 1. Reiniciando DAW limpa...")
...
if found_hwnd:
    ...
    # Clicar no botão 'Psy Bass' no menu do cockpit (x=740, y=16) para garantir foco na frente
    rect = wintypes.RECT()
    user32.GetWindowRect(found_hwnd, ctypes.byref(rect))
    mouse_click(rect.left + 740, rect.top + 16)
    time.sleep(1.0)
    
    # Capturar screenshot cristalina
    shot = trigger_and_save("21_kuro_psy_rolling_bass_engine_exact_mockup.png")
    print(f"[TEST] Screenshot capturada com sucesso: {shot}")
