...
# 1. Finalizar instâncias anteriores
...
if found_hwnd:
    ...
    # 2. Clicar no botão 'Piano' no Topbar (coordenada calibrada exata: rect.left + 672, rect.top + 42)
    smooth_click(rect.left + 672, rect.top + 42, "Botao Piano no Topbar (ocultar Piano Roll)")
    time.sleep(1.0)
    shot2 = trigger_and_save("02_channel_rack_shown.png")
    
    # 3. Clicar novamente no botão 'Piano' para reabrir
    smooth_click(rect.left + 672, rect.top + 42, "Botao Piano no Topbar (reabrir Piano Roll)")
    time.sleep(1.0)
    shot3 = trigger_and_save("03_piano_roll_reopened.png")
    
    print("[TEST] Concluído com sucesso! DAW aberta para uso interativo.")
else:
