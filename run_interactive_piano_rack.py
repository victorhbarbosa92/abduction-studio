...
if found_hwnd:
    ...
    shot1 = trigger_and_save("01_piano_roll_visible.png")
    
    # 2. Clicar no botão 'Piano' no Topbar (x=rect.left + 566, y=rect.top + 45)
    smooth_click(rect.left + 566, rect.top + 45, "Botao Piano no Topbar (ocultar para ver Channel Rack)")
    time.sleep(1.0)
    shot2 = trigger_and_save("02_channel_rack_visible.png")
    
    # 3. Clicar novamente no botão 'Piano' para reabrir o Piano Roll
    smooth_click(rect.left + 566, rect.top + 45, "Botao Piano no Topbar (reabrir Piano Roll)")
    time.sleep(1.0)
    shot3 = trigger_and_save("03_piano_roll_and_rack_both.png")
    
    print("[TEST] Validação completa com sucesso! A DAW permanece aberta.")
else:
