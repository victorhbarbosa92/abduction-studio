...
def patch_playlist_ui():
    ...
    if "DrawIconZeroCross" not in code:
...
"""
        code = code.replace("static void DrawIconFit(ImDrawList* draw, ImVec2 c, float sz, ImU32 col) {", zc_icons + "\n        static void DrawIconFit(ImDrawList* draw, ImVec2 c, float sz, ImU32 col) {")

    # 3. Replace renderModernToolbar
    start_marker = "void renderModernToolbar(float bpm, float bar_len_sec) {"
    end_marker = "void handleTimelineZoomAndScroll"
    idx_start = code.find(start_marker)
    idx_end = code.find(end_marker)

    if idx_start == -1 or idx_end == -1:
        print("ERROR: could not find renderModernToolbar boundaries!")
        return
    ...
    code = code[:idx_start] + new_toolbar + code[idx_end:]
    print("renderModernToolbar patched successfully!")

    # 4. Replace renderClipsForTrack
    rc_start_marker = "template<typename FSnap>\n        void renderClipsForTrack(ImDrawList* draw, int t, float ty0, float ty1, float grid_start_x, float grid_view_w, float time_to_px, float bar_len_sec, ImU32 trk_color, float snap_sec, FSnap snap_time, bool is_hovered) {"
    rc_end_marker = "void renderLaserPlayhead"
    rc_idx_start = code.find(rc_start_marker)
    rc_idx_end = code.find(rc_end_marker)

    if rc_idx_start == -1 or rc_idx_end == -1:
        print("ERROR: could not find renderClipsForTrack boundaries!")
        return
    ...
    new_render_clips = """template<typename FSnap>
        void renderClipsForTrack(ImDrawList* draw, int t, float ty0, float ty1, float grid_start_x, float grid_view_w, float time_to_px, float bar_len_sec, ImU32 trk_color, float snap_sec, FSnap snap_time, bool is_hovered) {
            ...
            for (int i = 0; i < (int)a_clips.size(); ++i) {
                ...
                // Container arredondado com borda estilizada
                ...
                // Header com nome do arquivo de áudio
                ...
                // Configuração da Forma de Onda Estéreo (Dual Lane: L no topo, R em baixo)
                ...
                // Linhas centrais sutis para cada canal e separador estéreo central
                ...
                for (float px = draw_x0; px < draw_x1; px += step_px) {
                    ...
                    if (ds.loaded && !ds.sample_data.empty() && ds.total_frames > 0) {
                        ...
                        // Modelagem de onda estéreo densa, contínua e orgânica (estilo WAV real de DAW)
            ...
            for (int i = 0; i < (int)m_clips.size(); ++i) {
                ...
                if (p) {
                    ...
                    // Garante uma extensão de afinação confortável (pelo menos 1 oitava)
                    ...
                    for (float loop_start = 0.0f; loop_start < mc.length_sec; loop_start += pat_len) {
                        // Linha divisória vertical suave de cada compasso dentro do clipe
                        ...
                        for (int ch : target_channels) {
                            for (const auto& note : p->getChannelNotes(ch)) {
                                ...
                                // NOTAS MIDI BRILHANTES EM CIANO NEON COM BRILHO AURA E NÚCLEO BRANCO
                                // 1. Brilho Aura Ciano
    ...
    code = code[:rc_idx_start] + new_render_clips + code[rc_idx_end:]
    print("renderClipsForTrack patched successfully!")
