# -*- coding: utf-8 -*-
"""
Script to generate the upgraded KuroDJStudioUI.h with:
1. Rekordbox 3-Band stacked horizontal rolling waveforms (Blue/Amber/White, Bar lines, dynamic FX glow)
2. 4-Decks native UI toggles (Deck 1|3 and Deck 2|4) with distinct colors (Cyan, Magenta, Amber, Green) and keyboard shortcuts (1, 2, 3, 4)
3. DJM-V10 Beat FX & Sound Color FX 12-mode selection matrix
4. USB / Pen Drive Export drawer with Rekordbox XML + Universal format
5. Zero vertical scrollbars on 1280x720
"""
...
namespace KuroUI {
    ...
    public:
        ...
        char search_query_buf[128] = "Vintage Culture";
        ...
        void init(KuroAudio::DJEngine* engine) {
            ...
            // Carrega faixas cacheadas imediatamente
        ...
        void render(KuroAudio::DJEngine& engine, bool* p_open = nullptr) {
            ...
            if (ImGui::Begin("##AbductionDJStudioPro", p_open, flags)) {
                ...
                renderStackedRekordboxWaveforms(engine, top_wave_w, 64.0f);
                ImGui::Spacing();

                // --- 3. GAVETA MODAL DE EXPORTAÇÃO PEN DRIVE USB ---
                if (show_export_modal) {
                    renderUSBExportDrawer(engine);
                    ImGui::Spacing();
                }

                // --- 4. GAVETA SPOTIFY / CLOUD TRACKS (TOPO) ---
                if (show_playlist_drawer) {
                    renderPlaylistSpotifyBar(engine);
                    ImGui::Spacing();
                }

                // --- 5. CORPO CENTRAL (DECK ESQUERDO ATIVO | MIXER DJM-V10 | DECK DIREITO ATIVO) ---
                ...
                ImGui::BeginChild("##DeckLeft_Child", ImVec2(deck_w, avail_h), false, child_flags);
                ...
                ImGui::BeginChild("##Mixer_Child", ImVec2(mixer_w, avail_h), false, child_flags);
                ...
                ImGui::BeginChild("##DeckRight_Child", ImVec2(deck_w, avail_h), false, child_flags);
    ...
    private:
        void renderTopHUD(KuroAudio::DJEngine& engine, bool* p_open) {
            ...
            // Ações do Topo: Pen Drive / Playlist / Tela Cheia / Sair
            ImGui::SameLine(avail_w - 440);
            ...
            if (ImGui::Button(ICON_FA_FLOPPY_DISK " PEN DRIVE USB")) {
            ...
            ImGui::SameLine(0, 6);
            if (ImGui::Button(show_playlist_drawer ? ICON_FA_LIST " Ocultar Biblioteca" : ICON_FA_LIST " Ver Biblioteca")) {
            ...
            ImGui::SameLine(0, 6);
            ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.35f, 0.08f, 0.12f, 0.9f));
        ...
        void renderStackedRekordboxWaveforms(KuroAudio::DJEngine& engine, float w, float h) {
            ...
            for (int lane = 0; lane < 2; lane++) {
                ...
                // 1. Fundo Obsidian da Lane
                dl->AddRectFilled(l_pos, ImVec2(l_pos.x + w, l_pos.y + lane_h), IM_COL32(5, 7, 11, 255), 3.0f);

                // 2. Efeito de Brilho Dinâmico (Glow Aura) quando CFX ou Beat FX está ligado
                bool fx_active = (std::abs(deck.filter_bipolar - 0.5f) > 0.04f) || 
                                 (engine.beat_fx.enabled && (engine.beat_fx.channel_assign == lane || engine.beat_fx.channel_assign == 2));
                if (fx_active) {
                    float pulse = (std::sin(ImGui::GetTime() * 10.0f) + 1.0f) * 0.5f;
                    ImU32 glow_col = ImGui::GetColorU32(ImVec4(theme_col.x, theme_col.y, theme_col.z, 0.4f + pulse * 0.45f));
                    dl->AddRect(l_pos, ImVec2(l_pos.x + w, l_pos.y + lane_h), glow_col, 3.0f, 0, 2.0f);
                } else {
                    dl->AddRect(l_pos, ImVec2(l_pos.x + w, l_pos.y + lane_h), IM_COL32(26, 36, 52, 180), 3.0f, 0, 1.0f);
                }

                // Linha de centro neutra
                float mid_y = l_pos.y + lane_h * 0.5f;
                dl->AddLine(ImVec2(l_pos.x, mid_y), ImVec2(l_pos.x + w, mid_y), IM_COL32(22, 30, 44, 150), 1.0f);
                ...
                if (!deck.buffer_l.empty() && deck.sample_rate > 1000.0) {
                    ...
                    int window_frames = (int)(deck.sample_rate * 3.8); // 3.8 segundos de janela
                    int start_f = (int)deck.current_frame - window_frames / 2;

                    // Beat Grid Sincronizado (Linha Vermelha na cabeça de compasso 4/4, branca nas batidas)
                    if (deck.bpm > 20.0) {
                        ...
                        if (beat_len > 10.0) {
                            ...
                            for (int b = b_start; b <= b_end; b++) {
                                ...
                                if (bx >= l_pos.x && bx <= l_pos.x + w) {
                                    bool is_bar = (b % 4 == 0);
                                    if (is_bar) {
                                        // Linha Vermelha de Compasso
                                        dl->AddLine(ImVec2(bx, l_pos.y), ImVec2(bx, l_pos.y + lane_h), IM_COL32(255, 45, 60, 230), 1.5f);
                                        dl->AddTriangleFilled(ImVec2(bx - 3.0f, l_pos.y), ImVec2(bx + 3.0f, l_pos.y), ImVec2(bx, l_pos.y + 4.0f), IM_COL32(255, 45, 60, 255));
                                    } else {
                                        dl->AddLine(ImVec2(bx, l_pos.y + 2.0f), ImVec2(bx, l_pos.y + lane_h - 2.0f), IM_COL32(230, 240, 255, 65), 1.0f);
                                    }
                                }
                            }
                        }
                    ...
                    // Cores 3-Band Rekordbox com modulação quando FX está ativo
                    float fx_shift = fx_active ? ((std::sin(ImGui::GetTime() * 12.0f) + 1.0f) * 0.25f) : 0.0f;
                    ImU32 col_high = ImGui::GetColorU32(ImVec4(0.00f + fx_shift, 0.65f + fx_shift, 1.00f, 0.92f)); // Agudos (Azul / Ciano)
                    ImU32 col_mid  = ImGui::GetColorU32(ImVec4(1.00f, 0.55f + fx_shift, 0.00f, 0.95f));              // Médios (Laranja / Âmbar)
                    ImU32 col_low  = IM_COL32(255, 255, 255, 255);                                                   // Graves / Kick (Branco Intenso)
                    ...
                    for (int x = 0; x < (int)w; x += step_px) {
                        ...
                        if (f_idx >= 0 && f_idx < (int)total_f) {
                            ...
                            peak = std::clamp(peak, 0.015f, 1.0f);

                            float total_amp = peak * (lane_h * 0.46f);
                            float mid_amp   = total_amp * 0.68f;
                            float low_amp   = total_amp * 0.38f;
                    ...
                    for (int c = 0; c < 8; c++) {
                        if (deck.hot_cues[c].active) {
                            ...
                            if (cx >= l_pos.x && cx <= l_pos.x + w) {
                                dl->AddLine(ImVec2(cx, l_pos.y), ImVec2(cx, l_pos.y + lane_h), deck.hot_cues[c].color_rgba, 2.0f);
                                dl->AddRectFilled(ImVec2(cx - 5, l_pos.y), ImVec2(cx + 5, l_pos.y + 11), deck.hot_cues[c].color_rgba, 2.0f);
                                char c_lbl[4];
                                snprintf(c_lbl, sizeof(c_lbl), "%d", c + 1);
                                dl->AddText(ImVec2(cx - 3, l_pos.y), IM_COL32(0, 0, 0, 255), c_lbl);
                            }
                        }
                    }
                }
                ...
                // Contador de Compassos estilo Rekordbox (ex: 121.2Bars)
                if (deck.bpm > 20.0 && deck.sample_rate > 1000.0) {
                    double beat_len = (60.0 / deck.bpm) * deck.sample_rate;
                    float bars = (float)(deck.current_frame / (beat_len * 4.0));
                    char bars_text[32];
                    snprintf(bars_text, sizeof(bars_text), "%.1fBars", bars);
                    dl->AddText(ImVec2(l_pos.x + w - 75, l_pos.y + 3), IM_COL32(0, 230, 255, 230), bars_text);
                }
            }

            // 5. Agulha Playhead Central Branca Laser (Alinhada atravessando ambas as pistas)
        ...
        void renderPlaylistSpotifyBar(KuroAudio::DJEngine& engine) {
            ...
            ImGui::PushStyleColor(ImGuiCol_ChildBg, ImVec4(0.05f, 0.07f, 0.10f, 0.96f));
            ImGui::PushStyleColor(ImGuiCol_Border, ImVec4(0.12f, 0.18f, 0.28f, 0.8f));
            ImGui::PushStyleVar(ImGuiStyleVar_ChildRounding, 6.0f);
            ImGui::BeginChild("##SpotifyPlaylistDrawer", ImVec2(avail_w, 140), true);

            // Linha de Busca Cloud Tracks
            ImGui::AlignTextToFramePadding();
            ImGui::TextColored(ImVec4(0.00f, 0.92f, 0.75f, 1.0f), ICON_FA_CLOUD_ARROW_DOWN " CLOUD TRACKS:");
            ImGui::SameLine();
            ...
            ImGui::SameLine();
            if (ImGui::Button(ICON_FA_MAGNIFYING_GLASS " PESQUISAR")) {
                spotify_service.searchAsync(search_query_buf);
            }

            // Importar Local
            ImGui::SameLine(0, 8);
            ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.12f, 0.35f, 0.45f, 0.9f));
            if (ImGui::Button(ICON_FA_FOLDER_PLUS " + IMPORTAR COMPUTADOR")) {
            ...
            // Quick Chips
            ImGui::SameLine(0, 10);
            auto chip = [&](const char* label) {
                if (ImGui::SmallButton(label)) {
                    strncpy_s(search_query_buf, label, sizeof(search_query_buf) - 1);
                    spotify_service.searchAsync(label);
                }
                ImGui::SameLine(0, 4);
            };
            chip("Psytrance"); chip("Vintage Culture"); chip("Fisher"); chip("Mau P"); chip("Charlotte de Witte");

            ImGui::SameLine();
            ImGui::TextColored(ImVec4(0.6f, 0.85f, 1.0f, 1.0f), "%s", spotify_service.getStatus().c_str());

            ImGui::Separator();

            // Tabela de Faixas
            auto track_list = spotify_service.getTracks();
            if (ImGui::BeginTable("##SpotifyTracksTable", 6, ImGuiTableFlags_ScrollY | ImGuiTableFlags_RowBg | ImGuiTableFlags_BordersInnerH, ImVec2(0, 95))) {
                ImGui::TableSetupColumn("Capa", ImGuiTableColumnFlags_WidthFixed, 36.0f);
                ImGui::TableSetupColumn("Título e Artista", ImGuiTableColumnFlags_WidthStretch);
                ImGui::TableSetupColumn("BPM", ImGuiTableColumnFlags_WidthFixed, 65.0f);
                ImGui::TableSetupColumn("Tom", ImGuiTableColumnFlags_WidthFixed, 75.0f);
                ImGui::TableSetupColumn("Duração", ImGuiTableColumnFlags_WidthFixed, 65.0f);
                ImGui::TableSetupColumn("Carregar nos Decks", ImGuiTableColumnFlags_WidthFixed, 205.0f);
                ...
                for (int i = 0; i < (int)track_list.size(); i++) {
                    ...
                    if (t.gl_tex_id != 0) {
                        ...
                        ImGui::TextColored(ImVec4(0.4f, 0.5f, 0.6f, 1.0f), ICON_FA_COMPACT_DISC);
                    }

                    ImGui::TableSetColumnIndex(1);
                    ImGui::TextColored(ImVec4(1.0f, 1.0f, 1.0f, 1.0f), "%s", t.title.c_str());
                    ImGui::SameLine();
                    ImGui::TextColored(ImVec4(0.6f, 0.65f, 0.75f, 1.0f), "• %s", t.artist.c_str());

                    ImGui::TableSetColumnIndex(2);
                    ImGui::TextColored(col_deck_1, "%.1f", t.bpm);

                    ImGui::TableSetColumnIndex(3);
                    ImGui::TextColored(ImVec4(1.0f, 0.85f, 0.35f, 1.0f), "%s", t.key.c_str());

                    ImGui::TableSetColumnIndex(4);
                    int min = (int)t.duration_sec / 60;
                    int sec = (int)t.duration_sec % 60;
                    ImGui::TextColored(ImVec4(0.7f, 0.7f, 0.7f, 1.0f), "%02d:%02d", min, sec);
                    ...
                    if (ImGui::SmallButton(t.is_downloading ? "Baixando..." : btn_l)) {
                        spotify_service.loadTrackIntoDeckAsync(i, left_id, engine);
                    }
                    ImGui::PopStyleColor();
                    ...
                    if (ImGui::SmallButton(t.is_downloading ? "Baixando..." : btn_r)) {
                        spotify_service.loadTrackIntoDeckAsync(i, right_id, engine);
                    }
                    ImGui::PopStyleColor();
                    ImGui::PopID();
                }
        ...
        void renderDeckPanel(KuroAudio::DJDeck& deck, int deck_slot, ImVec4 theme_col, KuroAudio::DJEngine& engine) {
            ...
            // --- MINI OVERVIEW WAVEFORM STRIP (NEEDLE SEARCH) ---
            renderMiniOverviewStrip(deck, w, 22.0f, theme_col);
            ImGui::Spacing();
        ...
        void renderMiniOverviewStrip(KuroAudio::DJDeck& deck, float w, float h, ImVec4 theme_col) {
            ...
            if (!deck.overview_peaks.empty()) {
                ImU32 col_mini = ImGui::GetColorU32(ImVec4(theme_col.x * 0.7f, theme_col.y * 0.7f, theme_col.z * 0.7f, 0.7f));
                size_t num_peaks = deck.overview_peaks.size();
                for (float x = pos.x; x < pos.x + w; x += 3.0f) {
                    int p_idx = (int)((x - pos.x) / w * (float)num_peaks);
                    if (p_idx >= 0 && p_idx < (int)num_peaks) {
                        float val = deck.overview_peaks[p_idx] * (h * 0.45f);
                        dl->AddLine(ImVec2(x, pos.y + h * 0.5f - val), ImVec2(x, pos.y + h * 0.5f + val), col_mini, 1.2f);
                    }
                }
            ...
            // Clique para saltar agulha
            ImGui::SetCursorScreenPos(pos);
            if (ImGui::InvisibleButton("##NeedleSearchStrip", ImVec2(w, h))) {
                float mouse_x = ImGui::GetMousePos().x - pos.x;
                float progress = std::clamp(mouse_x / w, 0.0f, 1.0f);
                deck.current_frame = progress * deck.duration_sec * deck.sample_rate;
            }
        ...
        void renderGiantVinylTurntable(KuroAudio::DJDeck& deck, ImVec2 top_left, float size, ImVec4 theme_col) {
            ...
            // Pino Central Metálico (Spindle)
            dl->AddCircleFilled(center, 8.0f, IM_COL32(210, 220, 230, 255), 24);
            dl->AddCircle(center, 8.0f, IM_COL32(100, 110, 120, 255), 24, 1.0f);
            dl->AddCircleFilled(center, 3.5f, IM_COL32(30, 35, 45, 255), 16);

            // Marcador visual de rotação do prato
            ...
            // 7. Interação de Mouse no Vinil (Scratch e Nudge)
            ImGui::SetCursorScreenPos(top_left);
            if (ImGui::InvisibleButton("##VinylTurntable", ImVec2(size, size))) {}
            ...
            if (active) {
                ...
                if (dist < radius * 0.90f) { // Platter Scratch
                    deck.jog_touch = true;
                    float rot_delta = (delta.x * (mouse.y - center.y) - delta.y * (mouse.x - center.x)) * 0.003f;
                    deck.target_scratch_velocity = rot_delta * 3.5f;
                    deck.scratch_active_ticks = 4;
                    deck.jog_visual_angle += rot_delta;
                } else if (dist < radius * 1.15f) { // Rim Nudge
                    float rot_delta = (delta.x * (mouse.y - center.y) - delta.y * (mouse.x - center.x)) * 0.001f;
        ...
        void renderCentralMixer(KuroAudio::DJEngine& engine) {
            ...
            ImGui::Combo("##FXType", &engine.beat_fx.type, fx_names, 5);
            ImGui::SameLine(0, 6);
            ImGui::SetNextItemWidth(half_combo);
            ImGui::Combo("##FXAssign", &engine.beat_fx.channel_assign, ch_assigns, 3);
...
with open(r"src/ui/KuroDJStudioUI.h", "w", encoding="utf-8") as f:
    f.write(code)

print("KuroDJStudioUI.h updated successfully!")
